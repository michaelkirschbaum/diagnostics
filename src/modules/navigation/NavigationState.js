...
const initialState = fromJS({
    ...
    HomeTab: {
        index: 0,
        routes: [{key: 'Login', title: 'CARFIT'}]
    },
    // Scenes for the `Counter` pages.
    CounterTab: {
        index: 0,
        routes: [{key: 'Counter', title: 'Counter for Test'}]
    },
...
export default function NavigationReducer(state = initialState, action) {
    switch (action.type) {
        case PUSH_ROUTE: {
            ...
            const tabKey = tabs.getIn(['routes', tabs.get('index')]).get('key'); // Returns the key using the index stored in the tab.
